...
在构建 Chrome 扩展之前，我们先构建了一个完整的 **跨平台桌面应用**，使用 Tauri（Rust + React），通过捕获系统音频并经过 8 级纯 Rust DSP 管线进行实时处理。桌面版作为独立仓库发布：**[sleep-mode-desktop](https://github.com/Yida-Dev/sleep-mode-desktop)**。

### 我们构建了什么

桌面方案让我们可以实现浏览器 AudioWorklet（限制在 48kHz 的 128 采样块）中不可能实现的 DSP 算法：

- **8 级纯 Rust DSP 管线**（4400+ 行纯 Rust 代码，无外部 DSP 库）：人声分离器、前瞻限制器、RMS 压缩器、响度标准化器、睡眠 EQ、时间拉伸、音调变换、真峰值限制器
- **WSOLA 时间拉伸**：标准的波形相似重叠相加算法，50% 重叠 Hann 窗，支持独立的速度和音调控制（在浏览器中不可能实现，我们只能依赖原生 `playbackRate`）
- **组合式变调**：不使用专用算法，变调 = 时间拉伸(0.841x) + 重采样(1.189x) 实现 -3 半音。优雅地复用现有模块
- **无 ML 的人声分离**：STFT 频谱掩蔽 + 频段交叉渐变（80-300Hz 渐入，300-3400Hz 人声频段，3400-8000Hz 渐出）。不到 1KB 代码，<5ms 延迟
- **macOS Core Audio Taps API**（macOS 14.2+）：无需任何虚拟音频驱动即可捕获系统音频 -- 不需要 BlackHole、不需要 Soundflower、不需要内核扩展
- **总延迟低于 10ms**：通过单元测试验证 8 级管线全链路延迟
- **平滑预设切换**：指数参数插值，~50ms 过渡时间，零可听不连续

### 为什么选择了扩展

| 因素 | 桌面应用 | Chrome 扩展 |
|------|---------|------------|
| YouTube 音频获取 | 间接（系统捕获） | 直接（`<video>` 元素） |
| 安装摩擦 | 下载 + 安装 + 权限配置 | 一键安装 |
| 权限复杂度 | CoreAudio/WASAPI/虚拟驱动 | 无 |
| DSP 能力 | 完整 Rust 管线，8 级 | AudioWorklet (JavaScript)，4 级 |
| 延迟 | < 10ms | ~15ms |

Chrome 扩展用 DSP 能力换取了简洁性。对于「YouTube 音频助眠」这个具体场景，直接访问元素 + 零安装摩擦，胜过系统级捕获 + 复杂的权限配置。

### 未解决的权限问题

桌面应用有几个重要的平台权限问题至今未解决：

- **macOS**：Core Audio Taps 需要 macOS 14.2+。聚合设备创建偶尔静默失败。ScreenCaptureKit 权限在不同 macOS 版本间表现不一致。
- **Windows**：VB-CABLE 虚拟音频设备需要手动安装（授权问题无法打包分发）。通过 `IPolicyConfig` 的系统音频重路由需要 UAC 处理。设备变更监听未实现。

这些权限问题是我们为 v1 选择 Chrome 扩展形态的重要因素。

### 未来计划

Rust DSP 模块功能完整且经过充分测试。如果未来重新考虑桌面形态（用于 Spotify、Apple Music 或其他非浏览器音频源），管线已经就绪。桌面版仓库作为参考实现和架构探索公开发布。
